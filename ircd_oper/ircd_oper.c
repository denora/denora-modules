      ...
      if (!aadmin) {
        ...
        if (ValidPassword(av[2], aadmin->passwd)) {
          ...
          if (aadmin->configfile == 1) {
            ...
            send_cmd(p10id, "SNO 1 :%s (%s@%s) is now an IRC Administrator (%s)", u->nick, u->username, u->host, aadmin->name);
          } else {
            send_cmd(p10id, "PRIVS %s %s", u->uid, PRIVSONOPER);
            send_cmd(p10id, "M %s %s", u->nick, MODESONOPER);
            send_cmd(p10id, "SNO 1 :%s (%s@%s) is now an IRC Operator (%s)", u->nick, u->username, u->host, aadmin->name);
          }
